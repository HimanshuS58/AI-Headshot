...
function finalizeImage(image: CloudinaryImage) {
    return image
...
export const HEADSHOT_PRESETS: HeadshotPreset[] = [
    {
        ...
        category: "style",
        build: (publicId) => finalizeImage(cld.image(publicId)
        .effect(
            generativeReplace()
            .from("casual shirt")
            .to("professional navy suit jacket with white dress shirt and tie")
        ))
        .effect(
            generativeBackgroundReplace().prompt(
                "modern corporate office with bookshelves professional lighting"
            )),
        transformationChain: "e_gen_replace:from_casual%20shirt;to_professional%20navy%20suit%20jacket%20with%20white%20dress%20shirt%20and%20tie/e_gen_background_replace:prompt_modern%20corporate%20office%20with%20bookshelves%20professional%20lighting/c_fill,w_600,h_750,g_auto/f_auto/q_auto",
    },
     {
